...
fn init() -> State{
    ...
    let todo_model = Rc::new(slint::VecModel::<TodoItem>::from(vec![
        TodoItem{checked: true, title: "English homework".into()},
    ]));
...
impl SerializedState{
    ...
    fn save(state: &State)-> Self{
        Self{
            tems:state.todo_model.iter().collect(),
...
fn press_add_adds_one_todo(){
    ...
    let line_edit = ElementQuery::from_root(&state.main_window).match_id("MainWindow::text-edit".find_first().unwrap());
    assert_eq!(line_edit.accessible_value().unwrap(), "");

    let button = ElementHandle::find_by_accessible_label(&state.main_window, "Add New Entry").next().unwrap();
